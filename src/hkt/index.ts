 ...
 */
export type TailCall<T, F> = T extends (...args: infer _Args) => infer R
	? R extends (...args: infer _Args2) => infer _R2
		? TailCall<R, F>
		: R
	: never
 ...
 */
export type Fix<F> = F extends (arg: infer _A) => infer R ? R : never
 ...
 * Mu - Mu fixpoint
 */
export interface Mu<F> { unfix: F extends (arg: infer A) => unknown ? A : never }

/**
 * Nu - Nu fixpoint
 */
export interface Nu<F> { out: F extends (arg: unknown) => infer R ? R : never }
 ...
 * Left - Left type
 */
export interface Left<L> { readonly _tag: 'Left', readonly left: L }

/**
 * Right - Right type
 */
export interface Right<R> { readonly _tag: 'Right', readonly right: R }
 ...
 */
export interface Nothing { readonly _tag: 'Nothing' }

/**
 * Just - Just type
 */
export interface Just<T> { readonly _tag: 'Just', readonly value: T }
